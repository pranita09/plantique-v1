
/**
 * Product Database can be added here.
 * You can add products of your wish with different attributes
 * */

export const products = [
  // flowering plants
  {
    _id: "aa5b428e-f8c9-47de-9714-867ea7fea3eb",
    title: "Kalanchoe Plant - Yellow",
    imgSrc: "/images/products/kalanchoe-plant-yellow.jpg",
    description: "Looking for a succulent that blooms? Look no further than the Kalanchoe Yellow. An evergreen succulent featuring ovate, glossy green leaves and striking flame-orange flowers, it will add a burst of colour and happiness to any space. True to its family genes, it thrives on very little care and makes a great choice for first time gardeners.",
    price: 400,
    updatedPrice: 300,
    starRating: 4.9,
    size: "Small",
    inStock: true,
    fastDelivery: false,
    onSale: false,
    category: 'Flowers',
  },
  {
    _id: "508f7462-a891-4d73-987e-ad030fa40fcd",
    title: "Peace Lily Plant",
    imgSrc: "/images/products/peace-lily-plant.jpg",
    description: "The bigger cousin of the beautiful peace lily, the Spathiphyllum Sensation is one of the most loved peace lily varieties in the world. In addition to its majestic white flowers, it has large, glossy, oblong leaves that are excellent air purifiers. The best part is its ease of growth and low-maintenance nature which makes it a perfect choice for first-time gardeners.",
    price: 500,
    updatedPrice: 350,
    starRating: 4.7,
    size: "Small",
    inStock: true,
    fastDelivery: true,
    onSale: true,
    category: 'Flowers',
  },
  {
    _id: "eaf91a20-5b28-4027-a6b2-bae97d76681a",
    title: "Poinsettia Plant",
    imgSrc: "/images/products/red-poinsettia-plant.jpg",
    description: "The Christmas plant carries all the festive cheer and traditional colours. The Poinsettia Plant instantly lifts up any room it is in and is the best Christmas décor money can buy. Colourful, with bright red, yellow and variegated leaves surrounding the buds amongst dark green leaves, the Poinsettias are the happiest and brightest in the months of December and January.",
    price: 700,
    updatedPrice: 650,
    starRating: 4.5,
    size: "Medium",
    inStock: true,
    fastDelivery: false,
    onSale: false,
    category: 'Flowers',
  },
  {
    _id: "d09da77f-9613-4b0a-bc52-f24dd7bf9903",
    title: "Ixora (Rugmini) Plant - Yellow",
    imgSrc: "/images/products/ixora-rugmini-plant-yellow.jpg",
    description: "Also known as Jungle Geraniums, Ixoras are small tropical shrubs that will reward you with beautiful clusters of flowers in bright yellow with very little care. Perfect for balcony gardens, these hardy plants need full sun and very little space to grow. The flowers bloom in a tight ball like cluster of tiny blooms that attract hummingbirds, butterflies, and bees with their fragrance.",
    price: 600,
    updatedPrice: 450,
    starRating: 3.1,
    size: "Small",
    inStock: true,
    fastDelivery: true,
    onSale: true,
    category: 'Flowers',
  },
  {
    _id: "1975fbb2-ddf4-45a5-a362-650ea7d56de0",
    title: "Ixora (Rugmini) Plant - Orange",
    imgSrc: "/images/products/ixora-rugmini-plant-dark-orange.jpg",
    description: "Also known as Jungle Geraniums, Ixoras are small tropical shrubs that will reward you with beautiful clusters of flowers in dark orange with very little care. Perfect for balcony gardens, these hardy plants need full sun and very little space to grow. The flowers bloom in a tight ball like cluster of tiny blooms that attract hummingbirds, butterflies, and bees with their fragrance.",
    price: 700,
    updatedPrice: 550,
    starRating: 4.7,
    size: "Medium",
    inStock: true,
    fastDelivery: true,
    onSale: false,
    category: 'Flowers',
  },
  {
    _id: "175ece04-a8b3-4cf5-a9f1-4de2c84c8b7c",
    title: "Aechmea Fasciata Plant",
    imgSrc: "/images/products/aechmea-fasciata-plant.jpg",
    description: "A flowering member of the bromeliad family, it is more popularly known as the Silver Vase or the Urn Plant. It sports leathery arched leaves, in mottled silver and sea green that are arranged in a rosette forming a watertight vase or urn, thus the common name. Easy to care for, it has blue blooms emerging from a bright pink bract. It is an excellent indoor plant if you're looking for a tropical plant that is a bit different.",
    price: 400,
    updatedPrice: 250,
    starRating: 2.8,
    size: "Medium",
    inStock: false,
    fastDelivery: false,
    onSale: false,
    category: 'Flowers',
  },
  {
    _id: "39475b82-0f43-40f1-a98f-a145c52c02db",
    title: "Kalanchoe Plant - Red",
    imgSrc: "/images/products/kalanchoe-plant-orange.jpg",
    description: "Looking for a succulent that blooms profusely? Kalanchoe is your answer. The Kalanchoe Red is a perennial succulent with thick leaves and eye-catching crimson-red flowers that grow in clusters through the year. Easy to grow and care for, the kalanchoe will fill your room with colour without much effort. All that it needs is a patch of sunshine.",
    price: 600,
    updatedPrice: 500,
    starRating: 4.1,
    size: "Small",
    inStock: false,
    fastDelivery: false,
    onSale: false,
    category: 'Flowers',
  },
  {
    _id: "203e6407-4fa8-47dc-8406-7ae2530c1461",
    title: "Anthurium Red Plant",
    imgSrc: "https://cdn.shopify.com/s/files/1/0579/7924/0580/products/medium-anthurium-red-plant-31798804512900.jpg?v=1675572474&width=750",
    description: "Anthurium Red or the Flamingo plant is a low-maintenance plant with heart-shaped red waxy blooms that last up to eight weeks. They bloom profusely for more than 250 days a year with bright indirect light and without any special attention and are the perfect size for any indoor area with their bright and shiny presence.",
    price: 1100,
    updatedPrice: 1000,
    starRating: 3.9,
    size: "Medium",
    inStock: true,
    fastDelivery: true,
    onSale: false,
    category: 'Flowers',
  },

  // Indoor Plants

  {
    _id: "ede18080-78ab-4c3e-a3e2-aded11c1bf95",
    title: "Aglaonema Red Plant",
    imgSrc: "/images/products/aglaonema-red-plant.jpg",
    description: "A great hassle free addition to indoor spaces to lend it colour and charm with its bright red leaves. A lover of indirect to low light areas and little water, this plant is the answer you were looking for to add some character to that empty corner with little light or your work area. Too much green in the plant cluster, add a spot of red and voila!",
    price: 1000,
    updatedPrice: 800,
    starRating: 2.5,
    size: "Medium",
    inStock: true,
    fastDelivery: false,
    onSale: true,
    category: 'Indoor',
  },
  {
    _id: "7f6e1427-c26d-4969-928f-ca966129a87a",
    title: "Christmas Cactus",
    imgSrc: "/images/products/christmas-cactus.jpg",
    description: "True to their name, these flat-leafed succulents bloom right in time for the holiday season and that too profusely. Also known as the Schlumbergera buckleyi, it's pretty pink blooms last well into the new year, and when not blooming they make for a pretty foliage plant. The Christmas cactus plant requires very little care other than bright light and occasional watering, they are excellent for those sunny windows of yours.",
    price: 800,
    updatedPrice: 700,
    starRating: 3.9,
    size: "Medium",
    inStock: true,
    fastDelivery: true,
    onSale: false,
    category: 'Indoor',
  },
  {
    _id: "37236383-7792-4a3d-afff-f6a624bc55a6",
    title: "Ficus Bonsai Plant",
    imgSrc: "/images/products/ficus-bonsai-plant.jpg",
    description: "The Ficus Bonsai, commonly known as the Weeping Fig or Ficus Tree is a beautiful houseplant which is loved by numerous plant parents. It has a unique curved trunk with dark green, oval leaves making it a brilliant centre piece for your living room or side table. Plant them in shallow but wide pots to show off its magnificent stem and roots.",
    price: 1100,
    updatedPrice: 1000,
    starRating: 4.5,
    size: "Medium",
    inStock: true,
    fastDelivery: false,
    onSale: true,
    category: 'Indoor',
  },
  {
    _id: "f0b613fb-81b4-47bc-9ca2-751a22977242",
    title: "Jade Plant Mini",
    imgSrc: "/images/products/jade-plant-mini.jpg",
    description: "Are you a sucker for succulents? The Crassula Green Mini will be your dream plant kid. One of the easiest houseplants to look after, the Crassula Green Mini boasts a lush foliage which beautifies any room. Also considered lucky as per Feng Shui for its coin like round plump leaves, so go on, bring some green home, the luck just tags along for free.",
    price: 700,
    updatedPrice: 500,
    starRating: 2.9,
    size: "Small",
    inStock: true,
    fastDelivery: true,
    onSale: false,
    category: 'Indoor',
  },
  {
    _id: "3cd5e18d-da71-4c4a-a3e5-46d529a8e039",
    title: "Syngonium Pink Plant",
    imgSrc: "/images/products/syngonium-pink-plant.jpg",
    description: "The Syngonium Pink Neon aka Syngonium podophyllum 'Neon Robusta' is simply the cutest ornamental creeper you will have met. Its adorable pink arrow-shaped leaves make him look so perfect that it feels it's handmade. Ideal for both tabletops and floor planters, with proper care, it can grow up to 3 to 6 feet. Also known as the Goosefoot or arrowhead plant, it is wonderfully unpredictable because as a baby he grows upright but can either trail or climb as he grows older.",
    price: 900,
    updatedPrice: 800,
    starRating: 4.9,
    size: "Medium",
    inStock: false,
    fastDelivery: false,
    onSale: false,
    category: 'Indoor',
  },

  // medicinal plants

  {
    _id: "d6c1f1e8-f8f2-46f2-9a6e-1886aa019479",
    title: "Aloe Vera Mini Plant",
    imgSrc: "https://cdn.shopify.com/s/files/1/0579/7924/0580/products/GroPot_a7daef94-5668-4941-8d88-03112485f554.jpg?v=1680411303&width=750",
    description: "The mini aloe vera has all the beauty of its bigger sister and none of the space requirements. With its green fleshy leaves that grow from the centre of the leaf cluster, it's a great addition to tight spaces. The aloe mini is a succulent that grows very well in indoor spaces with bright light, it is not medicinal in nature.",
    price: 1000,
    updatedPrice: 800,
    starRating: 4.8,
    size: "Medium",
    inStock: true,
    fastDelivery: true,
    onSale: true,
    category: 'Medicinal',
  },
  {
    _id: "92faab82-f28d-4a64-8f56-14e17f27cb16",
    title: "Rosemary Plant",
    imgSrc: "/images/products/rosemary-plant.jpg",
    description: "The Rosemary Plant has all the beauty of its bigger sister and none of the space requirements. With its green fleshy leaves that grow from the centre of the leaf cluster, it's a great addition to tight spaces. The aloe mini is a succulent that grows very well in indoor spaces with bright light, it is not medicinal in nature.",
    price: 700,
    updatedPrice: 500,
    starRating: 4.7,
    size: "Medium",
    inStock: true,
    fastDelivery: true,
    onSale: false,
    category: 'Medicinal',
  },

  {
    _id: "bb019e8d-f5d9-483d-8260-04d1ab280e77",
    title: "Lemon Grass",
    imgSrc: "https://cdn.shopify.com/s/files/1/0579/7924/0580/products/lemon-grass-31793410343044.jpg?v=1675599126&width=375",
    description: "If you are a tea lover then you know exactly the kind of freshness the Lemongrass brings to any setting. Native to Southeast Asia, the Lemongrass is a treasure trove of benefits. Excellent addition to your home gardens for its tall grass-like appearance, fresh lemony smell, and its ability to keep insects away, also this baby will look lush with very little care. You can use it to make your own cleaning spray or use it as a room freshener.",
    price: 500,
    updatedPrice: 250,
    starRating: 4.8,
    size: "Medium",
    inStock: true,
    fastDelivery: true,
    onSale: true,
    category: 'Medicinal',
  },

  // fruits plants

  {
    _id: "490c8e54-73d3-41da-a249-98f123a974a7",
    title: "Kesar Mango Plant",
    imgSrc: "/images/products/kesar-mango-plant.jpg",
    description: "Mango is a tropical fruit that is native to India and has been grown in the region for thousands of years. It is one of the most popular and beloved fruits in the country, and the mango plant is also an important part of Indian cuisine and culture.There are many different varieties of mango plants in India, including Alphonso, Kesar, Dasheri, Chausa, and Langra. Each variety has its own unique flavour, texture, and appearance.",
    price: 600,
    updatedPrice: 250,
    starRating: 4.6,
    size: "Medium",
    inStock: true,
    fastDelivery: true,
    onSale: true,
    category: 'Fruits',
  },
  {
    _id: "c6826efc-35a3-4996-a7eb-090a7b143d01",
    title: "Sapota Plant (Chiku)",
    imgSrc: "/images/products/sapota-plant-(chiku).jpg",
    description: "The chicoo plant (also known as sapodilla or sapota) is native to Mexico, Central America, and the Caribbean, and it has been grown in these regions for hundreds of years. It was introduced to India and other tropical countries by Spanish and Portuguese colonists, and it has become an important fruit crop in these regions. Chiku plants prefer a warm, tropical climate and well-draining soil. They also require plenty of sunlight and regular watering to thrive. In addition to planting from seed, chiku plants can also be propagated by rooting softwood cuttings or air layering.",
    price: 500,
    updatedPrice: 300,
    starRating: 3.6,
    size: "Medium",
    inStock: true,
    fastDelivery: false,
    onSale: false,
    category: 'Fruits',
  },

  // outdoor plants

  {
    _id: "d4ae83ad-39b8-4c04-8de0-f3492c3f5981",
    title: "Philodendron Melinonii Green",
    imgSrc: "/images/products/philodendron-melinonii-green.jpg",
    description: "A non-vining and big-leafed member of the philodendron family, the Philodendron melinonii green is a fuss-free plant. This dark green philodendron variety is an excellent choice if you are looking to add a tropical vibe to your indoor garden and looks equally stunning in shaded outdoor gardens. The leaves are large and a glossy dark green with clearly defined veins supported by a thick leaf stalk that can range in colour from light green to coppery red. In its natural habitat, the Philodendron melinonii can grow as an epiphyte on the top of trees forming a crown. A rare and new plant for Indian gardens, it is extremely easy to care for and loves the indian tropical growing conditions.",
    price: 500,
    updatedPrice: 300,
    starRating: 2.8,
    size: "Medium",
    inStock: false,
    fastDelivery: true,
    onSale: false,
    category: 'Outdoor',
  },
  {
    _id: "5c8b26ce-934c-4dc7-a09a-59d7a17b20f6",
    title: "Thuja Plant",
    imgSrc: "/images/products/thuja-plant-xl.jpg",
    description: "This sun-loving plant will add an evergreen patch to balconies and windows with the harshest sunlight. Excellent choice to provide that much-needed shielding from your intrusive neighbours, this conifer with its flat leaves requires very little care and can be pruned into almost any shape. Resilient in the face of pollution they will also trap most of the dust that your windows or balconies get.",
    price: 1600,
    updatedPrice: 1300,
    starRating: 4.2,
    size: "Medium",
    inStock: true,
    fastDelivery: true,
    onSale: true,
    category: 'Outdoor',
  },
  
  // TableTop plants
  
  {
    _id: "bb019e8d-f5d9-483d-8260-04d1ab280e77",
    title: "Lucky Bamboo Plant - 2",
    imgSrc: "https://cdn.shopify.com/s/files/1/0579/7924/0580/products/glass-bowl-lucky-bamboo-plant-2-layer-31793705713796.jpg?v=1675599478&width=375",
    description: "One of the luckiest plants according to Feng Shui is the Lucky Bamboo 2 Layer plant. This easy to grow and inexpensive plant is a popular choice of plant to keep at homes or offices. According to Feng Shui, the Lucky Bamboo brings positivity, wealth, and good health to homes.",
    price: 500,
    updatedPrice: 300,
    starRating: 4.8,
    size: "Small",
    inStock: true,
    fastDelivery: true,
    onSale: true,
    category: 'For TableTop',
  },

  {
    _id: "1cf3fc80-fa47-4186-8067-a53baffd4321",
    title: "Haworthia Fasciata Plant",
    imgSrc: "/images/products/haworthia-fasciata-plant.jpg",
    description: "These beautiful dainty succulents are one of the most attractive table top plants ever! With their rich green colour and white stripe like variegations they paint a mesmerising picture. The fleshy leaves grow in rosettes and their non-fussy nature makes them a great gift option.",
    price: 300,
    updatedPrice: 200,
    starRating: 3.5,
    size: "Small",
    inStock: true,
    fastDelivery: false,
    onSale: true,
    category: 'For TableTop',
  },
  {
    _id: "e5d08ca3-280f-4d59-a61b-636da2d5923f",
    title: "Green Fern Plant - Mini",
    imgSrc: "/images/products/green-fern-plant-mini.jpg",
    description: "Big things come in small packages - this holds especially true for the Green Fern Mini. A super-compact and cute as a button plant with miniature, crowded, droopy, and bright green foliage. It fits in with a majority of décor styles, but is a match made in heaven for Bohemian and Mid-century Modern interior design styles.",
    price: 200,
    updatedPrice: 150,
    starRating: 4.5,
    size: "Small",
    inStock: true,
    fastDelivery: true,
    onSale: false,
    category: 'For TableTop',
  },
  {
    _id: "af9ab5ba-7417-4498-ab2c-9d7571929fb8",
    title: "Earth Star Plant",
    imgSrc: "/images/products/earth-star-plant-pink.jpg",
    description: "A member of the bromeliad family, native to Brazil, the earth star brown is a short plant with tapering flat leaves arranged in rosettes. This low growing stunner boast of wavy leaves with a striped variegation. An excellent choice for table tops and shelves.",
    price: 400,
    updatedPrice: 200,
    starRating: 4.5,
    size: "Small",
    inStock: false,
    fastDelivery: true,
    onSale: true,
    category: 'For TableTop',
  },
  {
    _id: "e7b454cf-1e33-49e3-b794-f75971127708",
    title: "Snake Plant - Golden",
    imgSrc: "/images/products/snake-plant-golden.jpg",
    description: "A dwarf variety of the famed snake plant family, the Sansevieria Golden Hahnii sports beautiful variegated leaves in grey-green with dark green bands and golden yellow margins. It is a low maintenance plant that is hard to kill even for the most novice of gardeners. Its compact size makes it an excellent choice for office desks and small spaces.",
    price: 300,
    updatedPrice: 200,
    starRating: 4.7,
    size: "Small",
    inStock: true,
    fastDelivery: true,
    onSale: true,
    category: 'For TableTop',
  },
  {
    _id: "fc51a51a-41e2-4d75-87b9-0e011b40107f",
    title: "Haworthia Cymbiformis Plant",
    imgSrc: "/images/products/haworthia-cymbiformis-plant.jpg",
    description: "Native to South Africa, the Haworthia Cymbiformis literally means boat shaped. The green fleshy leaves grow in beautiful rosettes and are tipped with translucent skin that gives it a dewy appearance. Place them on your work desk to brighten up your work space.",
    price: 500,
    updatedPrice: 400,
    starRating: 4.9,
    size: "Small",
    inStock: true,
    fastDelivery: true,
    onSale: false,
    category: 'For TableTop',
  },
  {
    _id: "639e5fcf-4a7a-4878-836e-061a940c5c06",
    title: "Radermachera Bonsai Plant",
    imgSrc: "/images/products/radermachera-bonsai-plant.jpg",
    description: "The Radermachera bonsai is a stunning addition to any bonsai collection. With its delicate pink flowers and glossy green leaves, this plant is not only visually appealing but also easy to care for. As a slow-growing bonsai, it is perfect for those who are new to bonsai cultivation. Radermachera bonsai is an attractive bonsai option for those looking for an easy-to-care-for plant with a lovely appearance. With its pink flowers and green leaves, it will add a touch of nature to any indoor space. Ideal for beginners, this bonsai is a great choice for anyone looking to start a bonsai collection.",
    price: 1200,
    updatedPrice: 1000,
    starRating: 3.9,
    size: "Small",
    inStock: true,
    fastDelivery: false,
    onSale: false,
    category: 'For TableTop',
  },
  {
    _id: "b2408556-0120-40c1-ad71-0287e900a059",
    title: "Money Plant Satin",
    imgSrc: "/images/products/money-plant-satin.jpg",
    description: "Brighten your homes with the Money Plant Satin. The Money Plant Satin has large, silky, dark green leaves with beautiful silvery-white variegations that can make any home corner stand out. This plant is also very easy to grow and look after so all you brown thumbs, this is the plant you have been looking for all along!",
    price: 600,
    updatedPrice: 500,
    starRating: 4.2,
    size: "Small",
    inStock: true,
    fastDelivery: true,
    onSale: false,
    category: 'For TableTop',
  },
  {
    _id: "ff6ab9d1-30ee-45b8-afe7-95a0f3d40ecd",
    title: "The Sun-loving Succulent Bundle",
    imgSrc: "/images/products/the-sun-loving-succulent-bundle.jpg",
    description: "These are the sun babies! A perfect fit for the sunny corners of your home, they require very little care and thrive on neglect. These succulents have both pattern and form, their rosettes are a work of art. Place them on your sunny windowsills and admire their beauty without worrying about their care. An easy way to add green to your space that suits your busy lifestyle.",
    price: 1500,
    updatedPrice: 1200,
    starRating: 3.5,
    size: "Small",
    inStock: true,
    fastDelivery: false,
    onSale: true,
    category: 'For TableTop',
  },
  {
    _id: "df04070a-eff2-46a8-bfcf-ed3572d74983",
    title: "Haworthia Turgida Plant",
    imgSrc: "/images/products/haworthia-turgida.jpg",
    description: "A small succulent that grows in rosettes of glossy light green leaves with a crystalline texture. The succulent does very well indoors with bright indirect light and grows up to a size of 4 inches. The leaf tips are curved and translucent at the top with green striations. In ideal conditions, the succulents produce white tubular flowers on upright wiry stems.",
    price: 200,
    updatedPrice: 100,
    starRating: 3.5,
    size: "Small",
    inStock: true,
    fastDelivery: true,
    onSale: false,
    category: 'For TableTop',
  },
  {
    _id: "6fb42232-e0ce-4b0d-a571-039288178e0c",
    title: "The TableTop Succulent Bundle",
    imgSrc: "/images/products/the-tabletop-succulent-bundle.jpg",
    description: "Succulents are some of the easiest plants for bright spaces and what’s better than one succulent is a bundle of them. This bundle has the finest hand-picked succulents of beautiful shapes and colour for indoor spaces. Make your own succulent garden/arrangement and turn your old boring tabletop into an art piece. Just give these babies some bright light and they will reward you with blooms too.",
    price: 1200,
    updatedPrice: 900,
    starRating: 4.5,
    size: "Small",
    inStock: true,
    fastDelivery: false,
    onSale: true,
    category: 'For TableTop',
  },
  {
    _id: "b6586aef-5033-4562-b40b-7fbd2b2229b2",
    title: "Chlorophytum Spider Plant",
    imgSrc: "/images/products/chlorophytum-spider-plant.jpg",
    description: "Chlorophytum Spider aka the Spider Plant, is an impressive indoor plant and perfect for beginners. Famous for its variegated lance shaped leaves that emerge from the centre of the plant and spill over the sides of the planter giving it a full appearance. This NASA approved air purifier is a low maintenance plant, where the baby plants grow on thin stems emerging from the plant that can be removed and planted to grow new plants and share with friends. Its growing pattern makes it an excellent fit for hanging planters.",
    price: 700,
    updatedPrice: 500,
    starRating: 4.8,
    size: "Small",
    inStock: false,
    fastDelivery: false,
    onSale: false,
    category: 'For TableTop',
  },
  {
    _id: "8ffb6580-b74f-46ba-a143-8ea269497394",
    title: "ZZ Plant",
    imgSrc: "/images/products/zz-plant.jpg",
    description: "Topping the list of indestructible houseplants is the Zamioculcas Zamiifolia more popularly known as the ZZ plant. This plant is loved for his easy going and non-demanding nature. The wide, dark, glossy leaves instantly add life to any interior and he will be a happy trooper in the darkest corners of your home.",
    price: 600,
    updatedPrice: 500,
    starRating: 4.5,
    size: "Small",
    inStock: true,
    fastDelivery: true,
    onSale: true,
    category: 'For TableTop',
  },
  {
    _id: "15e5bc45-f341-4287-97f3-d9a1f845ff69",
    title: "Lucky Bamboo Plant",
    imgSrc: "/images/products/lucky-bamboo-plant.jpg",
    description: "One of the luckiest plants according to Feng Shui is the Lucky Bamboo plant. This easy to grow and inexpensive plant is a popular choice of plant to keep at homes or offices. An excellent and easy to maintain gifting option that is said to bring luck to the bearer. Place it in a stunning glass bowl and make it a centrepiece.",
    price: 500,
    updatedPrice: 400,
    starRating: 4.5,
    size: "Small",
    inStock: true,
    fastDelivery: true,
    onSale: false,
    category: 'For TableTop',
  },

  // Climbers

  {
    _id: "0766f04e-e0a1-4690-a79b-42318e4fcd2f",
    title: "Betel Leaf Plant - (Magai Paan)",
    imgSrc: "/images/products/betel-leaf-plant-magai-paan.jpg",
    description: "The Betel Leaf or Magai Paan is a creeper with glossy heart-shaped leaves that are popularly consumed in India. It is fairly easy to grow even for new plant parents with little care. Let it climb or trail and it will be a great addition to both your collection and food habit as it is known to cool the body and provides many nutrients.",
    price: 800,
    updatedPrice: 700,
    starRating: 4.8,
    size: "Small",
    inStock: true,
    fastDelivery: true,
    onSale: true,
    category: 'Climbers',
  },
  {
    _id: "15944966-efae-460e-8c2a-27856a35a3f5",
    title: "Dragon Bubble Plant",
    imgSrc: "/images/products/dragon-bubble-plant.jpg",
    description: "An extremely unique plant native to Southeast Asia, the D. nummularia Ideaminubu is popularly known as the Dragon Jade plant owing to its tightly knit overlapping green leaves that resemble dragon scale. The plant has small aerial roots and it loves to climb on moss poles and wooden branches owing to its epiphytic nature. It loves bright indirect light on windows and hates overwatering.",
    price: 500,
    updatedPrice: 400,
    starRating: 4.8,
    size: "Small",
    inStock: false,
    fastDelivery: false,
    onSale: false,
    category: 'Climbers'
  },
  {
    _id: "bf0f5707-10b9-44f3-8e0a-0fb5acef9b5b",
    title: "Money Plant - Golden",
    imgSrc: "/images/products/money-plant-golden.jpg",
    description: "A darling of home gardeners everywhere, the Money Plant Golden is famous for its never give up attitude. This plant can survive neglect and adapt to almost all growing conditions to fill your space with trailing foliage of bright golden-green coloured leaves in no time. An air purifier that can be styled both as a climbing and trailing plant, it can instantly lend an easy bohemian and tropical vibe to any space.",
    price: 400,
    updatedPrice: 200,
    starRating: 3.8,
    size: "Small",
    inStock: true,
    fastDelivery: false,
    onSale: true,
    category: 'Climbers',
  },
  {
    _id: "b26dcdc3-5d0e-4fd6-9e3f-76d4088dda61",
    title: "Philodendron Oxycardium Brasil",
    imgSrc: "/images/products/philodendron-oxycardium-brasil-with-hanging-pot.jpg",
    description: "Home gardeners are partial towards philodendrons, maybe because of their easy nature, beautiful foliage, or the sheer versatility with which they can be used in any space. Now, what is better than a simple philodendron, a variegated one! With its beautiful green and yellow variegations, the philodendron brasil will make space in both your home and heart.",
    price: 600,
    updatedPrice: 500,
    starRating: 3.8,
    size: "Small",
    inStock: true,
    fastDelivery: false,
    onSale: false,
    category: 'Climbers',
  },
  {
    _id: "7e63e8c2-ea57-4c36-9a83-fe5ff4626af5",
    title: "Philodendron Oxycardium Green",
    imgSrc: "/images/products/philodendron-oxycardium-green-with-hanging-pot.jpg",
    description: "The indoor house plant that has been in trend for generations now, the Philodendron Oxycardium Green can as well be called the original superstar of the plant world. Large glossy heart shaped leaves, delicate trailing vines that can be styled as you wish and easy and quick growing nature make it a must have. Let it trail from the hanging planter or make it climb a trellis - and voila, you have yourself a Pinterest worthy wall.",
    price: 400,
    updatedPrice: 300,
    starRating: 4.2,
    size: "Small",
    inStock: true,
    fastDelivery: false,
    onSale: true,
    category: 'Climbers',
  },
  {
    _id: "a9b35fc4-c94f-4c51-8e3d-0efe43119e22",
    title: "Twisted Hoya",
    imgSrc: "/images/products/twisted-hoya.jpg",
    description: "Hoya carnosa has twisted, waxy leaves that emerge on long rope-like vines, making this Hoya perfect as a hanging plant. Sometimes known by its nicknames, Hindu Rope Hoya and Indian Rope Hoya, it will eventually produce beautiful clusters of star-shaped pink flowers as it matures. Also called the Krinkle Curl plant and this conversation starter will add a unique touch to your home.",
    price: 300,
    updatedPrice: 100,
    starRating: 3.5,
    size: "Small",
    inStock: true,
    fastDelivery: true,
    onSale: true,
    category: 'Climbers',
  },
  {
    _id: "83c69369-7d18-4da4-a777-c9de6c615525",
    title: "Money Plant Variegated",
    imgSrc: "/images/products/money-plant-variegated.jpg",
    description: "Not an expert in growing plants? No problem. The Money Plant Variegated is perfect for all new gardeners. Featuring beautiful variegations on glossy, heart-shaped leaves, the Money Plant is an extremely low-maintenance plant that is ideal for both homes and offices. The variegated money plant, just like its family members is great for both indoor and outdoor spaces and requires minimal care.",
    price: 900,
    updatedPrice: 800,
    starRating: 4.3,
    size: "Small",
    inStock: true,
    fastDelivery: false,
    onSale: true,
    category: 'Climbers',
  },
];
